...
RSpec.describe VagrantPlugins::Tart::Action::StopInstance do
  ...
  describe "#call" do
    it("does not stop the instance when the instance does not exist") do
      ...
      expect(driver).not_to receive(:stop).with("name")
      expect(app).to receive(:call).with(env)

      sut.call(env)
    end

    it("does not stop the instance when the instance is nil") do
      allow(driver).to receive(:list)
    ...
    it("does not stop the instance when the instance is not running") do
      allow(driver).to receive(:list)
    ...
    it("stops the instance when the instance is running") do
      allow(driver).to receive(:list)
        .and_return(VagrantPlugins::Tart::Action::ActionSpecHelper.list_with_instance_running)
      allow(driver).to receive(:get)
        .with("name")
        .and_return(VagrantPlugins::Tart::Action::ActionSpecHelper.get_with_instance_running)
      expect(driver).to receive(:stop).with("name")
      expect(app).to receive(:call).with(env)
